/** Static assets exported from Figma (Hackhaton UMKT 2026 — Homepage). */
export const HOME_ASSETS = {
  /** Room only (no character) — pairs with centered doctor overlay */
  sceneBgMobile: "/assets/home/scene-bg.png",
  sceneBgDesktop: "/assets/home/scene-bg-desktop.png",
  doctorCharacter: "/assets/home/doctor-character-main.png",
  avatarDefault: "/assets/home/figma/avatar-frame.png",
  iconStars: "/assets/home/figma/icon-stars.png",
  iconTrophy: "/assets/home/figma/icon-trophy.png",
  iconWardrobe: "/assets/home/figma/icon-wardrobe.png",
  navIconHome: "/assets/home/figma/nav-icon-home.png",
  navIconHistory: "/assets/home/figma/icon-history.png",
  navIconSetting: "/assets/home/figma/nav-icon-setting.png",
} as const;
